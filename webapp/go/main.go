...
func postChair(c echo.Context) error {
	...
	_, err = tx.NamedExec("INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES(:id, :name, :description, :thumbnail, :price, :height, :width, :depth, :color, :features, :kind, :popularity, :stock)", chairs)
	if err != nil {
		c.Logger().Errorf("failed to insert chair: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT * FROM estate WHERE ST_Contains(ST_PolygonFromText(%s), geom) ORDER BY popularity DESC, id ASC LIMIT ?`, coordinates.coordinatesToText())
	...
	return c.JSON(http.StatusOK, EstateSearchResponse{int64(len(estatesInBounding)), estatesInBounding})
}
