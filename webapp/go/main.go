...
func (mc *MySQLConnectionEnv) ConnectDB(hostNumber int) (*sqlx.DB, error) {
	dsn := fmt.Sprintf("%v:%v@tcp(%v:%v)/%v", mc.User, mc.Password, mc.Host[hostNumber], mc.Port, mc.DBName)
...
func getEstateDetail(c echo.Context) error {
	...
	var estate Estate
	err = dbEstate.Get(&estate, "SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features FROM estate WHERE id = ?", id)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchEstates(c echo.Context) error {
	...
	searchQuery := "SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features FROM estate WHERE "
...
func getLowPricedEstate(c echo.Context) error {
	...
	query := `SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features FROM estate ORDER BY rent ASC, id ASC LIMIT ?`
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	query = `SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY popularity DESC, id ASC LIMIT ?`
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features FROM estate WHERE ST_Contains(ST_PolygonFromText(%s), geom) ORDER BY popularity DESC, id ASC LIMIT ?`, coordinates.coordinatesToText())
	...
	return c.JSON(http.StatusOK, EstateSearchResponse{Count: int64(len(estatesInBounding)), Estates: estatesInBounding})
}
